      ...
      <div className="flex-1 p-10 overflow-y-auto flex justify-center">
        <div className="w-full max-w-2xl">
          <div className="bg-white rounded-[40px] p-10 shadow-sm border-2 border-gray-50 flex flex-col items-center">
            ...
            <div className="relative mb-8 group cursor-pointer" onClick={() => fileInputRef.current?.click()}>
              <div className="w-32 h-32 rounded-full overflow-hidden border-4 border-[#2E9DF7] shadow-lg flex items-center justify-center bg-[#E0F2FE]">
                {currentUser.avatarBase64 ? (
                  ...
                  <span className="text-4xl font-bold text-[#2E9DF7]">{currentUser.name.substring(0,2).toUpperCase()}</span>
                )}
              </div>
              <div className="absolute inset-0 bg-black/40 rounded-full flex items-center justify-center opacity-0 group-hover:opacity-100 transition-opacity">
                <span className="text-white text-xs font-bold uppercase tracking-wider">Change</span>
              </div>
              <div className="absolute bottom-0 right-0 w-10 h-10 bg-[#F4511E] rounded-full flex items-center justify-center border-4 border-white shadow-sm pointer-events-none">
            ...
            <h3 className="text-2xl font-black text-[#2D2D2D] mb-1">{currentUser.name}</h3>
            <div className="flex items-center gap-2 mb-8">
              <span className="bg-[#E0F2FE] text-[#1E40AF] px-3 py-1 rounded-full text-xs font-bold uppercase tracking-wider">
