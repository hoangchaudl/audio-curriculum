...
export interface Notice {
  ...
  icon: 'alert' | 'assignment' | 'lesson' | 'calendar' | 'check';
  text: string;
  hash: string;
}
...
export const traineeNotifications = (
  ...
  for (const o of overdue) out.push({ id: `overdue:${o.hash}`, tone: 'red', category: 'deadlines', icon: 'alert', text: `${o.title} is overdue${o.due ? ` (was due ${when(o.due)})` : ''}`, hash: o.hash });
  ...
  (outline?.weeks ?? []).forEach((w, wi) => w.items.forEach(it => {
    ...
    out.push({ id: `due:${a.id}`, tone: 'orange', category: 'deadlines', icon: 'assignment', text: `${a.title} is due ${days === 0 ? 'today' : days === 1 ? 'tomorrow' : `in ${days} days`} (${when(due)})`, hash: `#/assignment/${a.id}` });
  }));

  for (const p of lessonPace(outline, assignments, enrollment.startDate, traineeId, videoProgress, now)) {
    out.push({ id: `pace:${p.week}`, tone: 'orange', category: 'lessons', icon: 'lesson', text: p.current ? `Week ${p.week}: ${p.done} of ${p.total} lessons done - you're behind pace` : `Week ${p.week} still has ${p.total - p.done} lessons to finish`, hash: '#/program/today' });
  }
  ...
  if (week) {
    ...
    if (planned.length) {
      out.push({ id: `today:${localDate(midnight)}`, tone: 'blue', category: 'lessons', icon: 'calendar', text: `Today's plan: ${planned.length} lesson${planned.length === 1 ? '' : 's'} - ${planned.join(', ')}`, hash: '#/program/today' });
    }
  }

  for (const key of ['episodeA', 'episodeB', 'da', 'pod'] as const) {
    if (publication?.[key]) out.push({ id: `published:${key}`, tone: 'green', category: 'results', icon: 'check', text: `${STAGE_NAMES[key]} results are published - see your score and feedback`, hash: '#/program/grades' });
  }
  return out;
};
